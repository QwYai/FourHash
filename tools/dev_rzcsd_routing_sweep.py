"""Predeclared indT-only routing sweep for the compact RZ-CSD encoder.

This follows the capacity sweep after larger/deeper MLPs failed.  It keeps the
frozen 40+5 curriculum and compact dimensions, and changes only how the two
modalities enter the shared semantic space, the residual activation, and the
hash projection.  Formal query/database artifacts are intentionally not CLI
arguments.
"""
...
def _train_variant(
    ...
    *,
    ...
    model = RoutingRZCSD512(
        label_dim=labels_u8.shape[1], config=config, variant=variant
    ).to(device)
    # Auxiliary decoder initialization must not perturb the inference model's
    # deterministic random stream relative to the exact compact control.
    ...
    for epoch in range(total_epochs):
        ...
        print(
            json.dumps(
                {"stage": "routing_epoch", "variant": variant.name, **record},
                sort_keys=True,
            ),
            flush=True,
        )
...
def run(fit_root: Path, output_dir: Path, *, seed: int, device: torch.device) -> dict[str, Any]:
    ...
    for variant in ROUTING_VARIANTS:
        ...
        body = {
            "schema": "raw_rebuilt_rzcsd_routing_candidate_indt_v1",
        ...
        torch.save(
            {
                ...
                "routing_variant": record["variant"],
                **state,
            },
            checkpoint,
        )
    ...
    if baseline is None:
        raise AssertionError("routing registry is empty")
    ...
    selection = {
        "schema": "raw_rebuilt_rzcsd_routing_sweep_indt_v1",
        "status": "DEVELOPMENT_ONLY_NOT_A_PAPER_CLAIM",
        "dataset": manifest["dataset"],
        "source_seal_sha256": manifest["source_seal_sha256"],
        "fit_artifact_sha256": manifest["fit_artifact_sha256"],
        ...
        "candidate_registry": [asdict(variant) for variant in ROUTING_VARIANTS],
        "selection_rule": (
